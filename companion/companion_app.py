...
from companion.plugin_manager import PythonPluginManager
...
def main():
    ...
    parser.add_argument("--plugins", action="store_true", help="List installed plugins")
    args = parser.parse_args()

    if args.plugins:
        pm = PythonPluginManager()
        plugins = pm.list_plugins()
        print(f"Installed CmdBar Plugins ({len(plugins)}):")
        for p in plugins:
            print(f" - {p['id']} (v{p['version']}): {p['name']} - {p['description']}")
        return
