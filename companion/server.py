...
class CmdBarHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/v1/config":
            ...
            shortcut_name = urllib.parse.unquote(
                path[len("/api/v1/shortcuts/") : -len("/execute")]
            )
    ...
    def _handle_monitoring(self):
        ...
        with METRICS_LOCK:
            ...
            avg_duration = (
                round((TOTAL_EXECUTION_DURATION / exec_cnt) * 1000, 2)
                if exec_cnt > 0
                else 0.0
            )
    ...
    def _handle_update_config(self, body: dict):
        ...
        if not isinstance(body, dict) or "categories" not in body:
            self._send_error_response(
                "Invalid configuration format. Must contain 'categories' array.", 400
            )
            return
    ...
    def _handle_add_shortcut(self, body: dict):
        ...
        if not name or not template:
            self._send_error_response(
                "Both 'name' and 'command'/'template' are required.", 400
            )
            return
        ...
        if save_config(config):
            ...
            self._send_json_response(
                {"success": True, "shortcut": new_cmd_obj}, status=201
            )
        else:
    ...
    def _handle_update_shortcut(self, name: str, body: dict):
        ...
        if save_config(config):
            broadcast_ws_event({"event": "config_updated", "data": config})
            self._send_json_response(
                {"success": True, "message": f"Shortcut '{name}' updated successfully"}
            )
        else:
    ...
    def _handle_delete_shortcut(self, name: str):
        ...
        if save_config(config):
            broadcast_ws_event({"event": "config_updated", "data": config})
            self._send_json_response(
                {"success": True, "message": f"Shortcut '{name}' deleted successfully"}
            )
        else:
            self._send_error_response(
                "Failed to save configuration after deletion", 500
            )
    ...
    def _handle_execute(self, body: dict):
        ...
        if not name and not command:
            self._send_error_response(
                "Either 'name' or 'command' parameter is required for execution.", 400
            )
            return
        ...
        if name:
            for cat in config.get("categories", []):
                for c in cat.get("commands", []):
                    if (
                        c.get("name") == name
                        or c.get("template") == name
                        or c.get("command") == name
                    ):
                        found_cmd_obj = c
                        break
                if found_cmd_obj:
        ...
        template = (
            found_cmd_obj.get("template", found_cmd_obj.get("command", command))
            if found_cmd_obj
            else (command or name)
        )
        ...
        for p_name, p_val in params.items():
            if p_name in param_rules and "regex" in param_rules[p_name]:
                ...
                if not validate_input(str(p_val), pat):
                    err_msg = param_rules[p_name].get(
                        "error_message",
                        f"Parameter '{p_name}' value fails validation pattern '{pat}'",
                    )
    ...
    def _handle_ai_translate(self, body: dict):
        ...
        try:
            ...
            self._send_json_response({"prompt": prompt, "command": translated})
        except Exception as e:
        ...
        try:
            while True:
                ...
                if frame.opcode == WebSocketFrame.OP_PING:
                    ...
                    pong_frame = WebSocketFrame.encode(
                        frame.payload, opcode=WebSocketFrame.OP_PONG
                    )
    ...
    def _process_ws_message(self, msg: dict):
        ...
        if action == "ping":
            ...
            config = load_config()
            res_frame = WebSocketFrame.encode(
                json.dumps({"event": "config", "data": config})
            )
            self.wfile.write(res_frame)
            self.wfile.flush()
            ...
            if save_config(new_config):
                ...
                res_frame = WebSocketFrame.encode(
                    json.dumps({"event": "config_updated", "success": True})
                )
                self.wfile.write(res_frame)
                self.wfile.flush()
            ...
            template = (
                found_cmd_obj.get("template", found_cmd_obj.get("command", cmd))
                if found_cmd_obj
                else (cmd or name)
            )
            final_cmd = substitute_and_quote_command(template, params)
            ...
            # Send started event
            start_evt = WebSocketFrame.encode(
                json.dumps({"event": "started", "name": name, "command": final_cmd})
            )
            ...
            comp_evt = WebSocketFrame.encode(
                json.dumps(
                    {
                        "event": "completed",
                        "name": name,
            ...
            res_frame = WebSocketFrame.encode(
                json.dumps({"event": "subscribed", "status": "ok"})
            )
            self.wfile.write(res_frame)
            self.wfile.flush()
...
class CmdBarServer:
    ...
    def __init__(
        self, host: str = "127.0.0.1", port: int = 8080, config_path: str = None
    ):
        """
    ...
    def start(self, block: bool = False):
        ...
        self.server = ReusableThreadingServer(
            (self.host, self.port), CmdBarHTTPRequestHandler
        )
...
def get_systemd_unit_content(
    host: str = "127.0.0.1", port: int = 8080, config_path: str = None
) -> str:
    """
...
def install_systemd_service(
    ...
    try:
        ...
        subprocess.run(
            ["systemctl", "--user", "enable", "cmdbar-server.service"], check=False
        )
        print(
            "Systemd user service enabled. Start it with: systemctl --user start cmdbar-server"
        )
    except Exception as e:
...
def uninstall_systemd_service() -> bool:
    ...
    try:
        subprocess.run(
            ["systemctl", "--user", "stop", "cmdbar-server.service"], check=False
        )
        subprocess.run(
            ["systemctl", "--user", "disable", "cmdbar-server.service"], check=False
        )
    except Exception:
        pass
...
def status_systemd_service() -> bool:
    ...
    if installed:
        ...
        try:
            res = subprocess.run(
                ["systemctl", "--user", "status", "cmdbar-server.service"],
                capture_output=True,
                text=True,
            )
...
def main():
    ...
    parser.add_argument(
        "--host",
        default=os.environ.get("CMDBAR_SERVER_HOST", "127.0.0.1"),
        help="Host address to bind (default: 127.0.0.1)",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.environ.get("CMDBAR_SERVER_PORT", 8080)),
        help="Port to listen on (default: 8080)",
    )
    parser.add_argument(
        "--config",
        default=os.environ.get("CMDBAR_CONFIG_PATH"),
        help="Path to config.json file",
    )
    parser.add_argument(
        "--install-service",
        action="store_true",
        help="Install systemd user service unit",
    )
    parser.add_argument(
        "--uninstall-service",
        action="store_true",
        help="Uninstall systemd user service unit",
    )
    parser.add_argument(
        "--status-service",
        action="store_true",
        help="Check status of systemd user service",
    )
    parser.add_argument(
        "--daemon", action="store_true", help="Run server in background daemon mode"
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
    ...
    if args.daemon:
        if hasattr(os, "fork"):
            ...
            if pid > 0:
                print(
                    f"CmdBar Server started in daemon mode (PID {pid}) on http://{args.host}:{args.port}"
                )
                sys.exit(0)
            os.setsid()
