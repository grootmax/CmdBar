...
- **YubiKey 2FA Authentication** – Enterprise hardware-backed 2FA security for sensitive commands (`sudo`, `rm -rf`, `aws ecs`, `kubectl delete`, `deploy`). Supports Touch-to-Confirm presence detection, Yubico OTP validation (44-char ModHex tokens), FIDO2/U2F assertion signature verification, single-use emergency recovery codes, and D-Bus integration.
