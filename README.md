...
- **Organization Workspaces** – Team collaboration with shared command palettes, role-based access control (`Admin`, `Member`, `Viewer`), security policy enforcement, usage analytics, tier-based billing management (`Free`, `Pro`, `Enterprise`), private package registries, and Slack webhook & slash command integrations
