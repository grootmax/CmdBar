...
- **Workspace-Specific Configs** – Different configurations per project auto-detected from working directory (`cwd`) or Git repository root (`.cmdbar.json` / `.cmdbar/config.json`). Comes with built-in templates (`node`, `python`, `rust`, `go`, `generic`) and smooth active workspace switching.
