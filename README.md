...
- **Event-Based Triggers** – Run automated actions on file changes (file watchers), git hook invocations (pre-commit, post-commit, pre-push), incoming HTTP webhooks with HMAC SHA-256 signature verification, and system events (timers, startup, suspend/resume) with flexible conditional logic
