...
def parse_javascript_file(filepath):
    ...
    for m in re.finditer(r'\b(?:const|let|var)\s+([a-zA-Z0-9_$]+)\s*=\s*(?:async\s*)?function\s*\(([^)]*)\)', content):
        declarations.append({
            "type": "function",
            "name": m.group(1),
            "signature": f"({m.group(2).strip()})",
    ...
    for d in declarations:
        ...
        if doc_start != -1:
            ...
            if doc_end != -1 and doc_end < len(prefix):
                ...
                between_clean = re.sub(r'\b(export|default|async|let|const|var)\b', '', between).strip()
        ...
        if not matched_jsdoc:
            m_line = re.search(r'((?:\s*//.*?\n)+)\s*(?:export\s+|default\s+|async\s+|let\s+|const\s+|var\s+)*$', prefix)
            if m_line:
                lines = []
                for line in m_line.group(1).splitlines():
                    ...
                    lines.append(line)
                docstring = "\n".join(lines).strip()
                if "@public" in m_line.group(1) or "visibility: public" in m_line.group(1):
                    visibility = "public"
...
def generate_api_markdown(entities, target_name):
    ...
    md.append(f"Welcome to the **{target_name.capitalize()}** API Reference page. This documentation is automatically extracted from Python and JavaScript source files based on access visibility tags.\n")
    
    if not by_file:
        md.append("*No public integration APIs found for this target.*" if target_name == "public" else "*No API entities found.*")
        return "\n".join(md)
...
def compile_docs(live_reload=False):
    ...
    for target_name, target_cfg in targets.items():
        ...
        if target_name == "public":
            filtered_entities = [ent for ent in all_entities if ent["visibility"] == "public"]
        else:
