...
describe("Lock-File Synchronization and Recovery Unit Tests", () => {
  ...
  describe("Schema and Parsing Validation", () => {
    ...
    test("should reject malformed configurations", () => {
      ...
      expect(
        validateConfigSchema({ categories: [{ name: "", commands: [] }] }),
      ).toBe(false);
      expect(
        validateConfigSchema({
          categories: [
            { name: "Test", commands: [{ name: "", command: "echo" }] },
          ],
        }),
      ).toBe(false);
    });
  ...
  describe("Atomic Write-and-Replace", () => {
    ...
    test("should fail to save if configuration does not match schema", async () => {
      const badConfig = {
        categories: [{ name: "Bad", commands: [{ name: "", command: "" }] }],
      };
  ...
  describe("Cooperative Lock-File Synchronization", () => {
    ...
    test("should automatically recover from stale lock files", async () => {
      ...
      fs.writeFileSync(
        lockPath,
        JSON.stringify({ pid: 1234, timestamp: Date.now() - 5000 }),
      );
  ...
  describe("Non-Destructive Memory Fallback on Corruption (Requirement 1 & 2)", () => {
    ...
    test("should preserve json structure that fails schema validation on disk and fall back to default in memory", async () => {
      ...
      const invalidSchemaObj = {
        categories: [{ name: "invalid-cmd", commands: [{ name: "foo" }] }],
      };
