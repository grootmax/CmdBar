...
describe('Dedicated Accessibility CI Workflow & Report Artifacts', () => {
    ...
    const workflowPath = path.join(rootDir, '.github', 'workflows', 'accessibility.yml');
    const prManagerPath = path.join(rootDir, '.github', 'workflows', 'auto-pr-manager.yml');
    ...
    test('Automated PR manager detects and enforces accessibility workflow status check', () => {
        ...
        expect(prManagerContent).toContain('Blocking merge due to accessibility compliance failure');
    });
});
