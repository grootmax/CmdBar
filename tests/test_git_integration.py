...
class TestGitIntegration(unittest.TestCase):
    def test_default_config_includes_git_category(self):
        ...
        self.assertIsNotNone(
            git_cat, "Git category should be present in DEFAULT_CONFIG"
        )

        commands = git_cat.get("commands", [])
        self.assertTrue(
            len(commands) >= 4, "Git category should contain at least 4 commands"
        )
    ...
    def test_resolve_command_preview_with_git_placeholders(self):
        ...
        resolved, errors = resolve_command_preview(
            cmd_template, "shell-quoted", param_values, {}
        )
