...
export function checkLineHasDocstring(line) {
  ...
  if (
    /@(public|param|returns|return|description|module|class|function|see|example|typedef|property|doc|type)\b/.test(
      content,
    ) ||
    ...
    statusOutput.split("\n").forEach((line) => {
      if (line.length > 3) {
        ...
        if (rawPath.includes("->")) {
          rawPath
            .split("->")
            .forEach((p) => files.add(normalizePath(p.trim())));
        } else {
...
export function checkFileHasDocstringEdits(filePath, rootDir = process.cwd()) {
  ...
  if (baseRef) {
    const branchDiff = runGit(
      `git diff -U0 ${baseRef}...HEAD -- "${filePath}"`,
    );
    if (branchDiff) {
...
export function checkDocFreshness(
  fileList = null,
  docstringChecker = null,
  rootDir = process.cwd(),
) {
  const files =
    fileList !== null ? fileList.map(normalizePath) : getGitDiffFiles(rootDir);
  ...
  if (modifiedSourceFiles.length === 0) {
    return {
      ...
      undocumentedFiles: [],
      message:
        "No modified application or extension source files found in Git diff.",
    };
  }
  ...
  for (const sourceFile of modifiedSourceFiles) {
    const hasDocstring = docstringChecker
      ? docstringChecker(sourceFile)
      : checkFileHasDocstringEdits(sourceFile, rootDir);
    if (!hasDocstring) {
  ...
  if (undocumentedFiles.length > 0) {
    return {
      ...
      undocumentedFiles,
      message:
        `Documentation drift detected! The following modified source files require documentation updates in 'docs/' (or 'README.md') or inline docstrings:\n` +
  ...
  return {
    ...
    undocumentedFiles: [],
    message:
      "All modified source files include inline docstring or comment updates.",
  };
}
...
describe("Centralized Documentation Structure & Build Automation", () => {
  ...
  test("Brainstorming folders and internal developer files must be omitted from the generated public HTML output", () => {
    ...
    const publicIndexContent = fs.readFileSync(
      path.join(publicOutputDir, "index.html"),
      "utf8",
    );
  ...
  test("The generated pages must have relative sidebar links and correct active navigation highlight", () => {
    const publicIndexContent = fs.readFileSync(
      path.join(buildDir, "public", "index.html"),
      "utf8",
    );
    const developerIndexContent = fs.readFileSync(
      path.join(buildDir, "developer", "index.html"),
      "utf8",
    );
  ...
  test("Doc Freshness Assertion: Running checkDocFreshness on branch with application code changes and no documentation updates results in a failed test", () => {
    const result = checkDocFreshness(
      ["app/main.py", "extension/extension.js"],
      () => false,
    );
    expect(result.passed).toBe(false);
    expect(result.undocumentedFiles).toEqual([
      "app/main.py",
  ...
  test("Doc Freshness Assertion: Running checkDocFreshness on branch with application code changes and documentation edits passes cleanly", () => {
    const resultWithDocs = checkDocFreshness(
      ["app/main.py", "docs/developer/index.md"],
      () => false,
    );
    expect(resultWithDocs.passed).toBe(true);
    expect(resultWithDocs.undocumentedFiles).toEqual([]);

    const resultWithReadme = checkDocFreshness(
      ["extension/extension.js", "README.md"],
      () => false,
    );
  ...
  test("Doc Freshness Assertion: Running checkDocFreshness on branch with application code changes and docstrings passes cleanly", () => {
    const resultWithDocstrings = checkDocFreshness(
      ["app/main.py"],
