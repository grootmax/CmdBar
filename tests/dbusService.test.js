...
const { loadConfig, saveConfig, getDefaultConfigPath } = await import('../extension/configSync.js');
...
describe('CmdBar DBus Service Unit Tests', () => {
  ...
  test('AddCommand adds a new command and reloads menu', async () => {
    const success = await service.AddCommand('New Task', 'echo task', 'Automation');
