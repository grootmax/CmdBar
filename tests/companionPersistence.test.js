...
describe("Atomic Companion Persistence Tests", () => {
  ...
  beforeEach(() => {
    ...
    testDir = path.join(
      os.tmpdir(),
      `cmdbar-test-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`,
    );
  ...
  describe("Atomic Writes (Synchronous)", () => {
    ...
    test("should clean up temporary file if writing fails", () => {
      ...
      expect(() => saveConfigAtomically(testData, testFile)).toThrow(
    ...
    test("should leave existing file unchanged when atomic write fails", () => {
      ...
      expect(() => saveConfigAtomically({ status: "new" }, testFile)).toThrow(
  ...
  describe("Atomic Writes (Asynchronous)", () => {
    ...
    test("should clean up temporary file if async writing fails", async () => {
      ...
      const writeSpy = jest
        ...
        .mockRejectedValue(new Error("Simulated async write failure"));

      await expect(
        saveConfigAtomicallyAsync(testData, testFile),
      ).rejects.toThrow("Simulated async write failure");
    ...
    test("should handle rapid consecutive saves without partial or corrupt reads", async () => {
      ...
      const readInterval = setInterval(() => {
        try {
          ...
          console.error(
            "Reader encountered syntax error during rapid writes:",
            e,
          );
