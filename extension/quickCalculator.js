...
/**
 * Checks if text is a calculator query (starts with '>', '=', or 'calc').
 */
...
/**
 * Cleans calculator query string by removing leading '>', '=', or 'calc'.
 */
...
export const getCalculatorExpression = cleanCalculatorQuery;

export function evaluateMathExpression(expr) {
  const query = cleanCalculatorQuery(expr);
  if (!query) {
    return {
      success: false,
      result: null,
  ...
  try {
    ...
    return {
      success: true,
      result: resNum,
      formatted: formatNumber(resNum),
    };
  } catch (e) {
    const calcRes = calculate(expr);
    ...
    let numVal = Number(calcRes.result);
    return {
      success: true,
      result: isNaN(numVal) ? calcRes.result : numVal,
      formatted: String(calcRes.result),
    };
  }
}
